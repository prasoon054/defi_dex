...
contract Arbitrage {
    ...
    function arbitrageBtoAtoB(uint256 amountBIn, uint256 minProfit) external returns (uint256 finalB, uint256 profit) {
        ...
        // Retrieve reserves from DEX2 and DEX1.
        (uint256 reserveA2, uint256 reserveB2) = IDEX(dex2).getReserves();
        (uint256 reserveA1, uint256 reserveB1) = IDEX(dex1).getReserves();

        // Simulate the first swap on DEX2: TokenB → TokenA.
        uint256 simulatedTokenA = getAmountOut(amountBIn, reserveB2, reserveA2);

        // Simulate the second swap on DEX1: TokenA → TokenB.
        uint256 simulatedFinalB = getAmountOut(simulatedTokenA, reserveA1, reserveB1);
        ...
        // Execute the second swap on DEX1: TokenA for TokenB.
        IDEX(dex1).swapTokenAForTokenB(tokenABalance);
